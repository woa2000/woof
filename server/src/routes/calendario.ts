...
router.get('/', requireAuth(), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (req.query.categoria_pet) {
      filters.categoria_pet = req.query.categoria_pet as string;
    }
    if (req.query.mes) {
      filters.mes = req.query.mes as string;
    }
    if (req.query.prioridade) {
      filters.prioridade = req.query.prioridade as 'baixa' | 'media' | 'alta';
    }
    if (req.query.tags) {
