...
router.get('/', requireAuth(), async (req, res) => {
    try {
      ...
      `)
        ...
        const eventosVeterinariosSugeridos = gerarSugestoesEventosVeterinarios(todosEventos || [], hoje.getMonth() + 1 // mês atual (1-12)
        );
...
// =====================================================
function gerarSugestoesEventosVeterinarios(eventosExistentes, mesAtual) {
    // Base de conhecimento veterinário brasileiro simplificada
    const eventosSazonaisBase = {
    ...
    const eventosPerdidos = eventosRecomendadosMes.filter(evento => {
        ...
        return !eventosExistentesNomes.some(existente => existente.includes(eventoLower.split(' ')[0]) ||
            eventoLower.includes(existente.split(' ')[0]));
    });
    return eventosPerdidos.slice(0, 3); // Máximo 3 sugestões
}
function gerarOportunidadesConteudo(eventosMes) {
...
function inferirTipoConteudo(categoria) {
    const mapeamento = {
...
function calcularTimingOtimo(dataInicio) {
    ...
    if (diasAteEvento <= 3)
        return 'Publicar em 1-2 dias';
    if (diasAteEvento <= 7)
        return 'Publicar em 3-4 dias';
    if (diasAteEvento <= 14)
        return 'Publicar em 1 semana';
    return 'Publicar em 2 semanas';
}
function gerarHashtagsPorCategoria(categoria) {
    const hashtags = {
...
function gerarCTAPorCategoria(categoria) {
    const ctas = {
...
//# sourceMappingURL=calendario-insights.js.map