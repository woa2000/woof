...
export default function ChapterEditor({ params, searchParams }: { 
  ...
  // Encontrar o manual atual baseado no ID
  const currentManual = manuals.find(manual => manual.id === id);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // Para o sistema de mocks, simular salvamento
      console.log('Simulando salvamento:', { chapterIndex: activeChapter, content: chapterContent });
      setHasChanges(false);
      alert('Capítulo salvo com sucesso! (Simulação)');
    } catch (error) {
